...
const PATHS = {
  sass: [],
  javascript: ["assets/js/custom/*.js", "!assets/js/custom/customizer.js"],
  phpcs: ["**/*.php", "!wpcs", "!wpcs/**"],
  pkg: [
    "**/*",
    "!**/node_modules/**",
...
function browserSyncInit(done) {
  const files = ["**/*.php", "assets/images/**/*.{png,jpg,gif}"];
  ...
  return gulp
    ...
    .pipe(
      $.notify(function(file) {
        ...
        const errors = file.jshint.results
          .map(function(data) {
            if (data.error) {
              return (
                "(" +
...
const cleanCss = function() {
  return del(["assets/css/styles.css", "assets/css/styles.css.map"]);
};
  ...
  function logFileChange(event) {
    ...
    console.log(
      "[" +
        ...
        "] " +
        fileName.magenta +
        " was changed, running tasks..."
    );
  }

  watch(["assets/scss/**/*.scss"], series(cleanCss, sass)).on(
    "change",
    function(event, stats) {
      logFileChange(event);
    }
  );
  watch(["assets/js/**/*.js"], series(cleanJavascript, javascript, lint)).on(
    "change",
    function(event) {
      logFileChange(event);
    }
